<script lang="ts">
  ...
  type ItemMeta = { name: string; texture: string };

  let itemData = $state<Record<string, ItemMeta> | null>(null);

  onMount(() => {
    import('../../../data/items.json').then((module) => {
      itemData = module.default as never as Record<string, ItemMeta>;
    });
  });
...
{#if !itemData}
  <div
    class="relative mt-4 grid w-full animate-pulse gap-0.5 overflow-hidden rounded-md border-2 border-transparent bg-zinc-950/10 dark:bg-white/10"
  ...
  >
    {#each Array.from({ length: size }, (_, i) => i) as index}
      <div class="group relative aspect-video w-full overflow-hidden rounded-xs bg-white/60 dark:bg-gray-900/60"></div>
    {/each}
    <div class="absolute top-1/2 left-1/2 aspect-square w-12 -translate-x-1/2 -translate-y-1/2 animate-spin rounded-full border-4 border-t-transparent"></div>
  </div>
{:else}
